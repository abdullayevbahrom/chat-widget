    ...
    <div x-data="{ sidebarOpen: false }" class="min-h-screen flex">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="mt-6 px-4">
                <div class="space-y-1">
                    <a href="/dashboard"
                        class="flex items-center gap-3 px-4 py-3 bg-white/10 rounded-xl text-white font-medium">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="/dashboard/projects"
                        class="flex items-center gap-3 px-4 py-3 text-white/70 hover:bg-white/10 rounded-xl transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="/dashboard/conversations"
                        ...
                        @if($stats['open_conversations'] > 0)
                            <span
                                class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $stats['open_conversations'] }}</span>
                        @endif
                    </a>
                </div>
            </nav>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="glass border-b border-gray-200 h-20 flex items-center justify-between px-6">
                ...
                <div class="flex items-center gap-4 ml-auto">
                    @php
                        $currentUser = auth()->guard('web')->check() ? auth()->guard('web')->user() : auth()->guard('tenant_user')->user();
                    @endphp
                    <div class="text-right">
                        <p class="text-sm font-semibold text-gray-900">{{ $currentUser->name }}</p>
                        <p class="text-xs text-gray-500">
                            {{ $currentUser->is_super_admin ? 'Super Admin' : ($tenant->name ?? 'No Tenant') }}</p>
                    </div>
                    <div
                        class="w-10 h-10 rounded-full bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-white font-bold">
                    ...
                    @else
                        <a href="{{ route('admin.logout') }}" class="text-gray-500 hover:text-red-600 transition-colors"
                            onclick="event.preventDefault(); this.closest('form')?.submit() || document.getElementById('admin-logout-form').submit();">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1">
                                </path>
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="glass rounded-2xl p-6 card-hover transition-all duration-300">
                        <div class="flex items-center justify-between">
                            ...
                            <div
                                class="w-14 h-14 rounded-xl flex items-center justify-center bg-gradient-to-br from-emerald-500 to-emerald-700">
                                <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="glass rounded-2xl p-6 mb-8">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="/dashboard/conversations"
                            class="text-sm font-medium text-brand-600 hover:text-brand-700 hover:underline">View All
                            →</a>
                    </div>
                        ...
                        <div class="space-y-3">
                            @foreach($recentConversations->take(5) as $conv)
                                <div
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center text-brand-700 font-semibold">
                                    ...
                                    <span
                                        class="px-3 py-1 text-xs font-medium rounded-full {{ $conv->status === 'open' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600' }}">
                                        {{ ucfirst($conv->status) }}
                                    </span>
                                </div>
                            @endforeach
                ...
                <div class="glass rounded-2xl p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="/dashboard/projects"
                            class="text-sm font-medium text-brand-600 hover:text-brand-700 hover:underline">Manage →</a>
                    </div>
                    @if($projects->count() > 0)
                                ...
                                <div class="p-4 rounded-xl border border-gray-200 hover:border-brand-300 transition-colors">
                                    <div class="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500">Key: {{ substr($project->widget_key, 0, 12) }}...
                                            </p>
                                        </div>
                    ...
                    @else
                        <p class="text-gray-500 text-center py-8">No projects yet. <a href="/dashboard/projects"
                                class="text-brand-600 hover:underline">Create your first project</a></p>
                    @endif
                </div>
            </main>
        </div>
