...
<div class="max-w-3xl mx-auto" x-data="{
    ...
    hasToken: {{ $project && $project->telegram_bot_token ? 'true' : 'false' }},
    hasChatId: {{ $project && $project->telegram_chat_id ? 'true' : 'false' }},
    ...
    get canSendTest() {
        return this.hasToken && this.hasChatId;
    }
}">
    <!-- Success Alert -->
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-500 mt-1">Configure Telegram bot for each project</p>
    </div>

    @if($projects->count() === 0)
        <div class="glass rounded-2xl shadow-lg p-8 text-center">
            <svg class="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path>
            </svg>
            <h3 class="text-lg font-medium text-gray-900 mb-2">No Projects Found</h3>
            <p class="text-gray-500 mb-4">Create a project first to configure Telegram bot settings.</p>
            <a href="{{ route('dashboard.projects.create') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-white font-semibold bg-gradient-to-r from-brand-500 to-brand-700 hover:opacity-95 transition-opacity">
                Create Project
            </a>
        </div>
    @else
        ...
        <div class="glass rounded-2xl shadow-lg p-6 mb-6">
            <label for="project_select" class="block text-sm font-medium text-gray-700 mb-2">Select Project</label>
            <select id="project_select" x-model="selectedProjectId" @change="window.location.href = '?project_id=' + $event.target.value"
                ...
                @foreach($projects as $proj)
                    <option value="{{ $proj->id }}" {{ $selectedProjectId == $proj->id ? 'selected' : '' }}>
                        {{ $proj->name }} ({{ $proj->telegram_bot_token ? '✅ Configured' : '⚠️ Not configured' }})
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <form action="{{ route('dashboard.telegram.update') }}" method="POST" class="space-y-6">
            ...
            <div class="glass rounded-2xl shadow-lg p-6">
                <h2 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Bot Configuration — {{ $project->name }}
                </h2>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                <button type="button" @click="document.getElementById('bot_token').type = document.getElementById('bot_token').type === 'password' ? 'text' : 'password'"
                    ...
                    <div>
                        ...
                        <input type="text" name="chat_id" id="chat_id"
                               value="{{ old('chat_id', $project->telegram_chat_id) }}"
            ...
            <div class="glass rounded-2xl shadow-lg p-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        <div class="relative">
                            <input type="checkbox" name="is_active" value="1" id="is_active"
                                   {{ old('is_active', $project->telegram_is_active) ? 'checked' : '' }}
                                   class="sr-only peer">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <!-- Test Message Button (only shown when token and chat_id are set) -->
                        <template x-if="canSendTest">
                            <button type="button" @click="sendTestMessage()"
                                ...
                                <span x-text="sendingTest ? 'Sending...' : 'Send Test Message'"></span>
                            </button>
                        </template>
                    </div>
                </div>
    ...
    <div class="mt-8 glass rounded-2xl shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white/50 rounded-xl p-4">
                ...
                <p class="text-sm text-gray-600">Enter the bot token and chat ID above, then click "Send Test Message" to verify</p>
            </div>
        </div>
    </div>
</div>
